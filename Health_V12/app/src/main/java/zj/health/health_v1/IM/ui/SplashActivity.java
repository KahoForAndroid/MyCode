...
import zj.health.health_v1.Activity.Login_Activity;
...
//                Toast.makeText(this,getString(R.string.login_error),Toast.LENGTH_SHORT).show();
                Log.d("main","id="+UserInfo.getInstance().getId()+"  "+"sig="+UserInfo.getInstance().getUserSig());
                LoginBusiness.loginIm(UserInfo.getInstance().getId(), UserInfo.getInstance().getUserSig(), this);
                break;
            default:
                Toast.makeText(this,getString(R.string.login_error),Toast.LENGTH_SHORT).show();
...
//        String sig = sp.getString("sig",null);
        Health_AppLocation.instance.users = dbUtils.SelectUsersDataFirst();
//        String userid = sp.getString("id",null);
//        Log.d("main",+","+sig);
        if(Health_AppLocation.instance.users!=null){
            Health_AppLocation.instance.Token = Health_AppLocation.instance.users.getToken();
            Health_AppLocation.instance.Icon = Constant.photo_IP+Health_AppLocation.instance.users.getIconUrl();
            UserInfo.getInstance().setId(Health_AppLocation.instance.users.getId());
            UserInfo.getInstance().setUserSig(Health_AppLocation.instance.users.getSig());
        }
        ...
        String id =  TLSService.getInstance().getLastUserIdentifier();
        if(UserInfo.getInstance().getId()!=null && UserInfo.getInstance().getUserSig()!=null){
            if(StringUtil.isEmpty(TIMManager.getInstance().getLoginUser())){
                navToHome();
            }else{
