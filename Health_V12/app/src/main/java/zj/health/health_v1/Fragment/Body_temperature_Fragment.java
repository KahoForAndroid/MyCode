...
public class Body_temperature_Fragment extends Fragment implements View.OnClickListener{

    private View view = null;
    private TimePickerView timePickerView = null;
    private EditText temperature_edit;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if(view == null){
            view = inflater.inflate(R.layout.body_temperature_fragment,container,false);
        }
        initView();
        BindListener();
        setData();
    ...
    private void initView(){
        ...
        temperature_edit = (EditText)view.findViewById(R.id.temperature_edit);
    ...
    private void Submit(JSONObject jsonObject){
        ...
        new PostUtils().JsonPost(Constant.post_temp,jsonObject,submitHandler, (BaseActivity) getActivity());
    }
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.commit_Button:
                if(temperature_edit.getText().toString().length()<=0){
                    Toast.makeText(getActivity(), getString(R.string.all_input_tips), Toast.LENGTH_SHORT).show();
                }else{

                    JSONObject jsonObject = new JSONObject();
                    try {
                        jsonObject.put("temp",temperature_edit.getText().toString());
