package zj.health.health_v1.IM.AuxiliaryUtil;
...
public class ChatInput extends RelativeLayout implements TextWatcher,View.OnClickListener {
    ...
    public ChatInput(Context context, AttributeSet attrs) {
        super(context, attrs);
        LayoutInflater.from(context).inflate(R.layout.new_chat_input, this);
    ...
    public void initView(){
        ...
        LinearLayout btn_health = (LinearLayout) findViewById(R.id.btn_health);
        btn_health.setOnClickListener(this);
        LinearLayout btn_body_report = (LinearLayout) findViewById(R.id.btn_body_report);
        btn_body_report.setOnClickListener(this);
        LinearLayout drug_lins = (LinearLayout)findViewById(R.id.drug_lins);
        drug_lins.setOnClickListener(this);
        LinearLayout End_the_chat = (LinearLayout) findViewById(R.id.End_the_chat);
        End_the_chat.setOnClickListener(this);

        if(IsDoctor){
            btn_health.setVisibility(View.GONE);
            btn_body_report.setVisibility(View.GONE);
            drug_lins.setVisibility(View.VISIBLE);
        }else{
            btn_health.setVisibility(View.VISIBLE);
            btn_body_report.setVisibility(View.VISIBLE);
            drug_lins.setVisibility(View.GONE);
        }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.btn_send){
            if(ChatCount<41){
                ...
                chatView.setIsDelete(true);
                CreateDialog dialog = new CreateDialog();
                dialog.MessageDailog2(context,
                        View.inflate(context,R.layout.main_popwindow2,null),
                        context.getString(R.string.thisChatisEnd));
            }
        }
        if (id == R.id.btn_add){
...
//                    VideoInputDialog.show(fragmentActivity.getSupportFragmentManager());
                    if (requestRtmp()) {
                        ...
                        if(requestStorage_READ(fragmentActivity) && requestStorage_WRITE(fragmentActivity)){
                            chatView.videoAction();
                        }else{
                            Toast.makeText(activity, "请先授权", Toast.LENGTH_SHORT).show();
                        }
                }
                }
            }
        ...
        if (id == R.id.drug_lins){
            if(ChatCount < 41){
                chatView.send_CustomElem(SEND_TO_DRUG);
            }else{
                chatView.setIsDelete(true);
                CreateDialog dialog = new CreateDialog();
                dialog.MessageDailog2(context,
        ...
        if (id == R.id.btn_health){
            if(ChatCount < 41){
                chatView.send_CustomElem(SEND_TO_HEALTH);
            }else{
                chatView.setIsDelete(true);
                CreateDialog dialog = new CreateDialog();
                dialog.MessageDailog2(context,
        ...
        if (id == R.id.btn_body_report){
            if(ChatCount < 41){
                chatView.send_CustomElem(SEND_TO_REPORT);
            }else{
                chatView.setIsDelete(true);
                CreateDialog dialog = new CreateDialog();
                dialog.MessageDailog2(context,
        ...
        if (id == R.id.End_the_chat){
            chatView.send_CustomElem(END_THE_CHAT);
        }
    }
    ...
    private boolean requestStorage_READ(Activity activity){
        if (afterM()){
    ...
    private boolean requestStorage_WRITE(Activity activity){
        if (afterM()){
            int hasPermission = activity.checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE);
            if (hasPermission != PackageManager.PERMISSION_GRANTED) {
                activity.requestPermissions(new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},
    ...
    private void CameraPermission_ForVersion_O(){
